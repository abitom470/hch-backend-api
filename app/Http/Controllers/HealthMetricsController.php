...
class HealthMetricController extends Controller
{
     ...
     * Get data for charts
     */
    public function getChartData($metricType, Request $request)
    {
        try {
            $user = Auth::user();
            if (!$user) return $this->unauthorizedResponse();
    ...
    /**
     * Get current status for dashboard cards
     */
    ...
    { 
        return response()->json(['success' => true, 'message' => 'Symptom recorded']); 
    }
